...
CreateThread(function()
    Config.LoadPlugin("postals", function(pluginConfig)
        ...
        if pluginConfig.enabled and locationsConfig ~= nil then
            ...
            if state ~= "started" then
                if state == "missing" then
                    errorLog(
                        ("[postals] The configured postals resource (%s) does not exist. Please check the name."):format(
                            pluginConfig.nearestPostalResourceName))
                    shouldStop = true
                elseif state == "stopped" then
                    warnLog(
                        ("[postals] The postals resource (%s) is not started. Please ensure it's started before clients conntect. This is only a warning. State: %s"):format(
                            pluginConfig.nearestPostalResourceName, state))
                else
                    errorLog(
                        ("[postals] The configured postals resource (%s) is in a bad state (%s). Please check it."):format(
                            pluginConfig.nearestPostalResourceName, state))
                    shouldStop = true
                end
            end
            ...
            CreateThread(function()
                ...
                if pluginConfig.useCustomPostalCodeFile ~= "" and pluginConfig.useCustomPostalCodeFile ~= nil then
                    postalFile = LoadResourceFile(GetCurrentResourceName(), ("plugins/postals/%s"):format(
                        pluginConfig.useCustomPostalCodeFile))
                else
        ...
        elseif locationsConfig == nil then
            errorLog(
                "ERROR: Postals plugin is loaded, but required locations plugin is not. This plugin will not function correctly!")
            pluginConfig.enabled = false
